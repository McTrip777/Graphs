...
def dft(world, start):
    # assigns a value to the starting position
    visited = {start: 'true'}
    # will contain all possible pathways from 000
    stack = [[start]]

    # while the stack still contains rooms 
    while len(stack):
        # print(stack)
        # takes the last array from the stack
        path = stack.pop()

        # takes the last item of the array 
        roomID = path[-1]
        
        # records room entered and the exit
        room = world.rooms[roomID]

        #records only the exit for the current room
        exits = room.getExits()
        ...
        for direction in exits:
            ...
            if next_room not in visited:
                # passes in current rooms directions and returns the opposite
                visited[next_room] = direction
                
                # increments through the rooms
        ...
        if len(nextPath):
            ...
            while path != stack[-1][:-1]:
                # print(path, 'path')
                # print(stack[-1][:-1], 'stack')
                direction = visited[path.pop()]
                # print(direction)
                previous = ''
